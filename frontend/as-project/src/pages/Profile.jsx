...
const Profile = () => {
    ...
    const handleDeleteAccount = async () => {
        ...
        try {
            ...
            // Wipe out client side storage and redirect safely
            logout();
            navigate('/', { replace: true });
        } catch (err) {
            setErrors({ global: 'Failed to complete transaction safely.' });
                    ...
                    <div className="flex flex-col sm:flex-row items-center gap-6">
                        ...
                        <div className="text-center sm:text-left space-y-1">
                            ...
                            <h2 className="text-2xl font-extrabold text-gray-900 uppercase tracking-tight">
                                {user?.first_name || user?.last_name ? `${user.first_name || ''} ${user.last_name || ''}`.trim() : 'Apex Member'}
                            </h2>
                            <p className="text-sm font-medium text-gray-500">@{user?.username || 'username'}</p>
                        </div>
                    </div>
                    ...
                    <form onSubmit={handleUpdateProfile} className="md:col-span-2 space-y-6">
                        <div className="rounded-2xl bg-white p-6 shadow-xl border border-gray-100 space-y-5">
                            <h3 className="text-base font-black uppercase tracking-wider text-gray-900 border-b border-gray-100 pb-3 flex justify-between items-center">
                                ...
                                {isEditing && <span className="text-xs text-red-500 font-bold normal-case animate-pulse">Editing Mode Active</span>}
                            </h3>
